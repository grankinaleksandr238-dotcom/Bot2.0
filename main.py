...
# Функции для мультиплеерной игры
...
async def cmd_start(message: types.Message):
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO users (user_id, username, first_name, joined_date, balance, reputation, total_spent, negative_balance, game_wins) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) ON CONFLICT (user_id) DO NOTHING",
                user_id, username, first_name, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), 0, 0, 0, 0, 0
            )
    except Exception as e:
...
async def profile_handler(message: types.Message):
    ...
    try:
        async with db_pool.acquire() as conn:
            row = await conn.fetchrow(
                "SELECT balance, reputation, total_spent, negative_balance, joined_date, theft_attempts, theft_success, theft_failed, theft_protected, casino_wins, casino_losses, guess_wins, guess_losses, game_wins FROM users WHERE user_id=$1",
                user_id
            )
        if row:
            balance, rep, spent, neg, joined, attempts, success, failed, protected, cw, cl, gw, gl, gwins = row['balance'], row['reputation'], row['total_spent'], row['negative_balance'], row['joined_date'], row['theft_attempts'], row['theft_success'], row['theft_failed'], row['theft_protected'], row['casino_wins'], row['casino_losses'], row['guess_wins'], row['guess_losses'], row['game_wins']
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            if success and victim_balance > 0:
                ...
                # Проверка на реферальную награду (15 успешных краж)
...
async def chat_gift(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        try:
            # Пытаемся получить список администраторов (если бот админ)
    ...
    await message.answer(
        ...
        f"📊 Осталось подгонов в этом чате на сегодня: {gift_limit - (gift_count + 1)}"
    )
...
# ===== МУЛЬТИПЛЕЕРНАЯ ИГРА "21" (полная реализация) =====
...
async def leave_room_callback(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        if game['status'] == 'waiting':
            # Выход до начала игры – просто удаляем игрока, ставка не списывалась
...
async def close_room_callback(callback: types.CallbackQuery):
    ...
    async with db_pool.acquire() as conn:
        ...
        if game['host_id'] != user_id:
            ...
            return
        # Удаляем игроков и комнату (ставки не возвращаются, так как они не списывались)
        await conn.execute("DELETE FROM game_players WHERE game_id=$1", game_id)
        await conn.execute("DELETE FROM multiplayer_games WHERE game_id=$1", game_id)
    await callback.message.edit_text("🏁 Комната закрыта. Ставки возвращены.")  # текст можно оставить для информирования
    await callback.answer()
...
async def dealer_turn(game_id: str):
    async with db_pool.acquire() as conn:
        ...
        # Выбираем только игроков, которые не остановились и не сдались (stopped = FALSE)
        ...
        # Также уведомляем тех, кто сдался (они уже получили уведомление при сдаче)
        # Очищаем комнату
        await conn.execute("DELETE FROM game_players WHERE game_id=$1", game_id)
        await conn.execute("DELETE FROM multiplayer_games WHERE game_id=$1", game_id)
...
async def reset_confirm(callback: types.CallbackQuery):
    ...
    try:
        async with db_pool.acquire() as conn:
            await conn.execute("UPDATE users SET balance=0, reputation=0, total_spent=0, theft_attempts=0, theft_success=0, theft_failed=0, theft_protected=0, last_theft_time=NULL, negative_balance=0, game_wins=0")
